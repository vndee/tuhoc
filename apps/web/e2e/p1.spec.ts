...
/** Nơi API THẬT lắng nghe — cùng phép tính với `playwright.config.ts`. (`s2`/`s3`/`s4.spec.ts` từng có bản riêng của phép tính này nữa; cả ba đã bị xoá — hai cái sau ở Task 16 của server-side pivot, `s2.spec.ts` ở Task 16 của Pha 2.) */
const API_ORIGIN = (
