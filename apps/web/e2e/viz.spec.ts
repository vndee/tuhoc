 ...
 *   - the DoD gate stays fast (seconds). This one walks all 44 chapters
 *     and is minutes. See the run duration recorded in the final fix
 *     report.
 *
 ...
 * courses/***REMOVED***/viz.js. A count would let a viz silently
...
/**
 * `home-hero` is registered in viz.js but referenced by no chapter — it
 * belongs to the original single-file shell's landing page, which the new
 * reader does not have. It is still one of the 59 the spec's gate names,
 * and "it isn't on any page" is not the same claim as "it still works", so
 * it is mounted directly into the live runtime instead of being excused
 * (see `mountThroughRuntime`). This list is spelled out rather than
 * derived so that a SECOND name quietly falling out of the chapters fails
 * the coverage assertion below rather than joining an ever-growing
 * exemption list.
 */
const NOT_REFERENCED_BY_ANY_CHAPTER = ['home-hero'];
...
test.describe('every visualization runs in the new reader (spec §10 exit gate)', () => {
  // 44 chapters, each a full page load of the reader plus KaTeX plus the
  // 172KB viz bundle, plus a pixel read per canvas. The per-test default
  // (90s, playwright.config.ts) is for the DoD gate's shape of work, not
  // this one's.
  test.setTimeout(15 * 60 * 1000);

  test('all 59 registered viz render in the reader and produce no console errors', async ({ page, request }) => {
    ...
    expect(chapters.length, 'the manifest should list every chapter of the course').toBe(44);
    ...
    expect(registered.length, 'viz.js should register 59 visualizations — the number the spec\'s own exit gate names').toBe(59);
    ...
    expect(
      missing,
      `these registered visualizations were never executed — either a chapter stopped referencing them, or ${NOT_REFERENCED_BY_ANY_CHAPTER.length > 0 ? 'they joined `home-hero` in being unreachable' : 'they became unreachable'}. Either way this suite no longer covers them:`,
    ).toEqual([]);
