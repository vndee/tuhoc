...
- **Name the protection you are giving up: CSRF.** `SameSite=Lax` is currently this API's *only* CSRF defense — there is no CSRF token, no Origin/Referer check, and no custom-header requirement anywhere in `apps/api`. Switching to `SameSite=None` removes that defense outright: any site the user visits could then issue a credentialed cross-site `POST /sync` or `POST /auth/logout` on their behalf. The blast radius here is a learner's own reading progress rather than money or identity, so this is a documented consequence rather than a blocker — but it is a real one, and "add `SameSite=None`" is not a complete change on its own. Designing the replacement (a double-submit token, or an Origin allowlist on state-changing routes) is out of scope for this runbook; do not ship the no-domain route without deciding which one you are adding.
