...
Route collection tiếp tục sort số mới trước, giữ Số 01. Route/slug/hash không đổi theo ngôn ngữ. CTA cuối số tìm `ly-thuyet-thong-tin` từ catalog thực; có thì link `/c/ly-thuyet-thong-tin`, không có hoặc API lỗi thì dùng `/courses`, không hứa một khoá không tồn tại.
